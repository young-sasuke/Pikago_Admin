...
const DISPATCH_READY_ALIASES = ['ready_to_dispatch', 'ready_for_delivery'] as const
...
export default function OrdersPage() {
  ...
  const openAssignmentDetails = async (order: PikagoOrder) => {
    try {
      const { data, error } = await supabase
        ...
        .maybeSingle()
      if (error || !data) {
        toast.info('No assignment found for this order yet')
        return
      }
      setSelectedAssignment(data as unknown as Assignment)
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex items-center gap-2 justify-end">
                          ...
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => openAssignmentDetails(order)}
                          ...
                          {order.order_status === 'assigned' && (
                            <Button
                              variant="default"
                              size="sm"
                              onClick={() => updateRiderStatus(order.id, 'picked_up')}
                            >
                          ...
                          {order.order_status === 'picked_up' && (
                            <Button
                              variant="default"
                              size="sm"
                              onClick={() => updateRiderStatus(order.id, 'reached')}
                            >
                          ...
                          {order.order_status === 'in_transit' && (
                            <Button
                              variant="default"
                              size="sm"
                              onClick={() => updateRiderStatus(order.id, 'delivered')}
                            >
